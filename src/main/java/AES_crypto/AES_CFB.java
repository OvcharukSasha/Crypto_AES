...
public class AES_CFB { //Cipher FeedBack
    ...
    public String encrypt(String filename, String KeyString, int[][] ivMatrix) throws IOException {
        ...
        while (isFileReadable) {
            ...
            inputRoundIV = matrixXOR(aesCipher.DoEncryption(inputRoundIV, keysHexMatrix), hexInputMatrix).clone();

            for (int k = 0; k < 4; k++) {
                for (int j = 0; j < 4; j++) {
                    sbOutput.append(Integer.toHexString(inputRoundIV[j][k]) + " ");
                }
            }
        }
